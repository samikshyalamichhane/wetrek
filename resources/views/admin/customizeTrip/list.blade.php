...
<div class="page-heading">
    <h1 class="page-title">Customize Trips</h1>
    <!-- <ol class="breadcrumb">
...
<div class="page-content fade-in-up">
    <div class="ibox">
        <div class="ibox-head">
            <div class="ibox-title">All Customize Trips</div>
            <div>
                <a class="btn btn-info btn-md" href="{{route('customize.create')}}">Trekking Package Inquery</a>
            </div>
        </div>
        ...
        <div class="ibox-body">
            <table id="example-table" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
                ...
                <tbody>
                  ...
                  @if($customizeTrip->count())
                    @foreach($customizeTrip as $key => $data)
                      ...
                      <td>{{$data->type}}</td>
                        ...
                        <td>
                            {{-- @if($bookingListModule['edit_access']==1 || $bookingListModule['full_access']==1) --}}
                                    <a href="{{route('customize.show', $data->id)}}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
                            {{-- @endif --}}
                            {{-- @if($bookingListModule['full_access']==1) --}}
                                    <form class="adjust-delete-button" action="{{route('customize.destroy', $data->id)}}" method="post">
                                        @csrf
                                        @method('delete')
