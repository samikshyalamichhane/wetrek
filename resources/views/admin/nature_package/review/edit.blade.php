      ...
      <div class="row">
          <div class="col-md-6">
              <div class="ibox">
                  ...
                  <form method="post" action="{{route('review.update', [$d_id, $detail->id])}}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                  <div class="ibox-body" style="">
                    ...
                    @endif
                          <div class="form-group">
                                <label>Name</label>
                                <input class="form-control" type="text" name="name" value="{{$detail->name}}" placeholder="Enter Name">
                            </div>
                             

                      <div class="form-group">
                          <label>Review</label>
                          <textarea class="form-control" name="review" rows="3" cols="80">{{$detail->review}}</textarea>
                      </div>
                          ...
                          <tbody>
                            ...
                            @if($review->count())
                              ...
                              <tr>
                                ...
                                <td>{!! str_limit($data->review, 200) !!}</td>
                                <td>{{$data->published == 1 ? 'Published' : 'Not Published'}}</td>

                                  <td>
                                      <a href="{{route('review.edit', [$d_id, $data->id])}}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>

                                        <form class="adjust-delete-button" action="{{route('review.destroy', [$d_id, $data->id])}}" method="post">
                                            @csrf
                                            @method('delete')
...
@include('admin.layouts._partials.ckdynamic', ['name' => 'review'])
@endpush
