    ...
    @include('admin.adventure_package.include.topnavbar')
      ...
      <div class="row">
          <div class="col-md-6">
              <div class="ibox">
                  <div class="ibox-head">
                      <div class="ibox-title">Edit Adventure Package Itinerary</div>
                  ...
                  <form method="post" action="{{route('adventurepackageitinerary.update', [$d_id, $detail->id])}}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                  ...
                  <div class="ibox-body">
                      <table class="table table-bordered table-hover">
                          ...
                          <tbody>
                            ...
                            @if($adventurepackageitinerary->count())
                              @foreach($adventurepackageitinerary as $key => $data)

                              <tr>
                                <!-- <td>{{++$key}}</td> -->
                                <td>{{$data->day_num}}</td>
                                <td>{{$data->title}}</td>
                                <td>{!! str_limit($data->activity_details, 200) !!}</td>

                                  <td>
                                      <a href="{{route('adventurepackageitinerary.edit', [$d_id, $data->id])}}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>

                                        <form class="adjust-delete-button" action="{{route('adventurepackageitinerary.destroy', [$d_id, $data->id])}}" method="post">
                                            @csrf
                                            @method('delete')
