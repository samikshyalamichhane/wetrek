...
class Payment extends ActionRequest
{
    ...
    public function ExecuteFormJose($booking,$mid,$api_key,$curr,$amt,$threeD,$success_url,$failed_url,$cancel_url,$backend_url): string
    {
        ...
        $signingKey = $this->GetPrivateKey(SecurityData::$MerchantSigningPrivateKey);
        $encryptingKey = $this->GetPublicKey(SecurityData::$PacoEncryptionPublicKey);
        // dd($stringPayload,$signingKey,$encryptingKey);
        $body = $this->EncryptPayload($stringPayload, $signingKey, $encryptingKey);
  
        // dd($body,SecurityData::$AccessToken);
        //third-party http client https://github.com/guzzle/guzzle
        $response = $this->client->post('api/1.0/Payment/prePaymentUi', [
            'headers' => [
                'Accept' => 'application/jose',
        ...
        $decryptingKey = $this->GetPrivateKey(SecurityData::$MerchantDecryptionPrivateKey);
        $signatureVerificationKey = $this->GetPublicKey(SecurityData::$PacoSigningPublicKey);
        // $responseBack = json_decode($this->DecryptToken($token, $decryptingKey, $signatureVerificationKey));
        // // dd($responseBack->response->Data->paymentIncompleteResult->orderNo);

        // $amount = $responseBack->response->Data->paymentIncompleteResult->transactionAmount;
        // if($responseBack->response->ApiResponse->ResponseCode == 'PC-B050000' && $responseBack->response->ApiResponse->ResponseCode == 'PC-B050001'){
        //     $order = BookingForm::findOrFail($responseBack->response->Data->paymentIncompleteResult->orderNo);
        //     $order->update([
        //         'payment_status' => 'paid',
        //         // 'payment_type' => 'hbl',
