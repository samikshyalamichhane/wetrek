...
trait HeliTourPackageMultipleImagesTrait

{
    ...
    public function getMultipleImages($id)
    {
        ...
        // dd($details);
        return view('admin.helitour_package.slider.slider', compact('details'))->with('helitourpackage_id', $id);
    }
    ...
    public function multipleImagesStore(Request $request)
    {
        ...
        if ($request->hasFile('sliderimages')) {
            ...
            for ($i = 0; $i < $array_len; $i++) {
                ...
                $package_path = public_path('images/helitour/package/multiple');
                $image_array[$i]->move($package_path, $new_image_name);

                $formInput['sliderimages'] = $new_image_name;
                $formInput['helitourpackage_id'] = $request->helitourpackage_id;
                $success = HelitourPackageSlider::create($formInput);
            }
            return redirect()->back()->with('success', 'HelitourPackage Images Uploaded Sucessfully');
    ...
    {
        $oldRecord = HelitourPackageSlider::findorfail($id);
        ...
        if ($imagename) {
            ...
            $multiple = public_path('images/helitour/package/multiple/') . $imagename;
        ...
        return redirect()->back()->with('success', 'HelitourPackage Images Deleted Successfuly.');
    }
