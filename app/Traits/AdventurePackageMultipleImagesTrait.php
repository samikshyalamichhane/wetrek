...
trait AdventurePackageMultipleImagesTrait
{
    ...
    public function getMultipleImages($id)
    {
        ...
        // dd($details);
        return view('admin.adventure_package.slider.slider', compact('details'))->with('adventurepackage_id', $id);
    }
    ...
    public function multipleImagesStore(Request $request)
    {
        ...
        if ($request->hasFile('sliderimages')) {
            ...
            for ($i = 0; $i < $array_len; $i++) {
                ...
                $package_path = public_path('images/adventure/package/multiple');
                $image_array[$i]->move($package_path, $new_image_name);

                $formInput['sliderimages'] = $new_image_name;
                $formInput['adventurepackage_id'] = $request->adventurepackage_id;
                $success = AdventurePackageSlider::create($formInput);
            }
            return redirect()->back()->with('success', 'AdventurePackage Images Uploaded Sucessfully');
    ...
    {
        $oldRecord = AdventurePackageSlider::findorfail($id);
        ...
        if ($imagename) {
            ...
            $multiple = public_path('images/adventure/package/multiple/') . $imagename;
        ...
        return redirect()->back()->with('success', 'AdventurePackage Images Deleted Successfuly.');
    }
